...
class C3D8(ElementBaseClass, ABC):
    ...
    def __init__(self, eid=None):
        ...
        self.K = np.mat(np.zeros([24, 24], dtype=float))  # 刚度矩阵
        ...
        for ri in range(2):
            for si in range(2):
                for ti in range(2):
                    ...
                    dNdr = np.mat(([(s + 1) * (-1 - t) / 8, (1 - r) * (1 + t) / 8, (1 - r) * (1 + s) / 8],
                    ...
                    B = np.mat([[B_pre[0, 0], 0, 0, B_pre[0, 1], 0, 0, B_pre[0, 2], 0, 0, B_pre[0, 3], 0, 0, B_pre[0, 4], 0, 0, B_pre[0, 5], 0, 0, B_pre[0, 6], 0, 0, B_pre[0, 7], 0, 0],
