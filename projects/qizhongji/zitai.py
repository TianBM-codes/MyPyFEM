...
mq1330 = MQ1330()
Abj = 43.224
data = mq1330.getPartTranslate(Abj, Abj + 30)
bijia_trans, bijia_theta, bijia_origin = data["bijia"]
bijia_o = QVector3D(*bijia_origin)

dalagan_trans, dalagan_theta, dalagan_origin = data["dalagan"]
dalagan_o = QVector3D(*dalagan_origin)

phl_trans, phl_theta, phl_origin = data["pinghengliang"]
phl_o = QVector3D(*phl_origin)

xlg_trans, xlg_theta, xlg_origin = data["xiaolagan"]
xlg_o = QVector3D(*xlg_origin)
xbl_trans, xbl_theta, xbl_origin = data["xiangbiliang"]
xbl_o = QVector3D(*xbl_origin)
ctj_trans, ctj_theta, ctj_origin = data["chitiaojia"]
ctj_o = QVector3D(*ctj_origin)

z_axis = QVector3D(0, 0, 1)


def CalculateNewXYZ(node_id, x, y, z):
    ...
    if node_id < 20000:
        ...
        臂架只绕指定点旋转
        """
        new_x, new_y, new_z = RotateByAxis(z_axis, bijia_o, bijia_theta * 180 / np.pi, old_xyz)
        return new_x, new_y, new_z
    elif 50000 < node_id < 70000:
        """
        大拉杆只绕指定点旋转
        """
        new_x, new_y, new_z = RotateByAxis(z_axis, dalagan_o, dalagan_theta * 180 / np.pi, old_xyz)
        return new_x, new_y, new_z
    elif 70000 < node_id < 90000:
        """
        平衡梁只绕指定点旋转
        """
        new_x, new_y, new_z = RotateByAxis(z_axis, phl_o, phl_theta * 180 / np.pi, old_xyz)
        return new_x, new_y, new_z
        ...
        """
        new_x, new_y, new_z = RotateByAxis(z_axis, bijia_o, bijia_theta * 180 / np.pi, old_xyz)
        xx, yy, zz = RotateByAxis(z_axis, xlg_o, (-bijia_theta + xlg_theta) * 180 / np.pi, np.array((new_x, new_y, new_z)))
        return xx, yy, zz
        ...
        """
        new_x, new_y, new_z = RotateByAxis(z_axis, bijia_o, bijia_theta * 180 / np.pi, old_xyz)
        xx, yy, zz = RotateByAxis(z_axis, xbl_o, (-bijia_theta + xbl_theta) * 180 / np.pi, np.array((new_x, new_y, new_z)))
        return xx, yy, zz
    elif 130000 < node_id:
        """
        齿条架
        """
        new_x, new_y, new_z = RotateByAxis(z_axis, bijia_o, bijia_theta * 180 / np.pi, old_xyz)
        xx, yy, zz = RotateByAxis(z_axis, ctj_o, (-bijia_theta + ctj_theta) * 180 / np.pi, np.array((new_x, new_y, new_z)))
        return xx, yy, zz
    else:
...
if __name__ == "__main__":
    mq1330 = MQ1330()
    Abj = 43.224
    data = mq1330.getPartTranslate(Abj, Abj + 1)
